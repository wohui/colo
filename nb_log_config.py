...
LOG_PATH = '/logs/colo'  # 默认的日志文件夹,如果不写明磁盘名，则是项目代码所在磁盘的根目录下的/pythonlogs
...
if os.name == 'posix':  # linux非root用户和mac用户无法操作 /pythonlogs 文件夹，没有权限，默认修改为   home/[username]  下面了。例如你的linux用户名是  xiaomin，那么默认会创建并在 /home/xiaomin/pythonlogs文件夹下写入日志文件。
    ...
    LOG_PATH = Path(home_path) / Path('logs/colo')  # linux mac 权限很严格，非root权限不能在/pythonlogs写入，修改一下默认值。
...
DING_TALK_TOKEN = '3d'  # 钉钉报警机器人

EMAIL_HOST = ('smtp.sohu.com', 465)
EMAIL_FROMADDR = 'aaa@sohu.com'  # 'matafyhotel-techl@matafy.com',
EMAIL_TOADDRS = ('com', 'com',)
EMAIL_CREDENTIALS = ('aaa', 'bbb')
...
MONGO_URL = 'mongodb://my@127.0.0.1:27016/admin'
